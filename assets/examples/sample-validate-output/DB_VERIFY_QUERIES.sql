...
SELECT * FROM (
SELECT 'S1' AS scenario_id, 'S1: Happy path — eligibility inquiry' AS scenario_description, 'BOB-SAMPLE-20260601-S1' AS client_reference_code, txn_status, txn_result_code, txn_result_description, internal_txn_desc, CASE WHEN COALESCE(txn_status, '') = 'SUCCESS' AND COALESCE(txn_result_code, '') = '000' THEN 'PASS' ELSE 'FAIL' END AS assert_result FROM transaction_audit WHERE client_reference_code='BOB-SAMPLE-20260601-S1' ORDER BY updated_on DESC LIMIT 1
) AS _bob_S1
UNION ALL
SELECT * FROM (
SELECT 'S2' AS scenario_id, 'S2: Integration — processor unit scope' AS scenario_description, 'BOB-SAMPLE-20260601-S2' AS client_reference_code, txn_status, txn_result_code, txn_result_description, internal_txn_desc, 'REVIEW' AS assert_result FROM transaction_audit WHERE client_reference_code='BOB-SAMPLE-20260601-S2' ORDER BY updated_on DESC LIMIT 1
) AS _bob_S2;
